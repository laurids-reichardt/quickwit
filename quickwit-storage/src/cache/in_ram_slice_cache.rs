...
mod tests {
    ...
    fn test_cache_edge_condition() {
        let cache = SliceCache::with_capacity_in_bytes(5, &COUNTERS.fast_field_cache);
        {
    ...
    fn test_cache_edge_unlimited_capacity() {
        let cache = SliceCache::with_infinite_capacity(&COUNTERS.fast_field_cache);
        {
            let data = OwnedBytes::new(&b"abc"[..]);
            cache.put(PathBuf::from("3"), 0..3, data);
            assert_eq!(cache.get(Path::new("3"), 0..3).unwrap(), &b"abc"[..]);
    ...
    #[test]
    fn test_cache() {
        let cache = SliceCache::with_capacity_in_bytes(10_000, &COUNTERS.fast_field_cache);
